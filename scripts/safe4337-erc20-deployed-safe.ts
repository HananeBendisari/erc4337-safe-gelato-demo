/**
 * FAILED - ERC20 UserOperation using deployed Safe 1/1
 * STATUS: ❌ FAILED - API configuration issues
 * PURPOSE: Attempt to use deployed Safe for ERC20 payments
 * 
 * This script attempted to use the deployed Safe 1/1 for ERC20 payments
 * but failed due to API configuration issues with Gelato Bundler.
 * 
 * RELATED WORKING SCRIPTS:
 * - safe4337-erc20-starter-kit.ts (✅ FUNCTIONAL - Safe Starter Kit)
 * - safe4337-erc20-paymaster.ts (❌ FAILED - Paymaster API issues)
 */
...
async function main() {
  console.log("=== Safe 1/1 ERC20 UserOperation ===");
  logNetworkInfo();
  ...
  try {
    console.log("Creating ERC20 UserOperation with Safe 1/1...");
    ...
    // Create UserOperation payload for Safe 1/1 with ERC20 payment
    const userOperation = {
      ...
      maxFeePerGas: "0x0", // Zero for ERC20 transactions
      maxPriorityFeePerGas: "0x0", // Zero for ERC20 transactions
    ...
    console.log("  - ERC20: true (Safe 1/1 pays with tokens)");
    console.log("");

    // Send to Gelato Bundler with ERC20 paymaster
    console.log("Sending to Gelato Bundler with ERC20 paymaster...");
    
    const bundlerUrl = `https://api.gelato.digital/bundlers/${sepolia.id}/rpc?apiKey=${process.env.GELATO_API_KEY}&erc20=true`;
    ...
    console.log("Safe 1/1 sent the UserOperation and paid for gas using ERC20 tokens");
  ...
  console.log("  Safe 1/1 paid gas with ERC20 tokens");
}
